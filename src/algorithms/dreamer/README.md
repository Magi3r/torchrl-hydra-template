...
optimisations behind independent flags. Defaults match
[NM512/r2dreamer](https://github.com/NM512/r2dreamer): what it enables is on
(`dreamer_config.compile` in `reduce-overhead` mode, `tf32`, `amp: fp16` with a
`GradScaler`, `buffer_config.pin_memory`, and `channels_last`, whose NHWC
activations r2dreamer got implicitly from its input permute), while this port's
own additions (`static_pad`, `dedup_value`, `cudnn_benchmark`,
`foreach_laprop`) default to `false`:
...
python src/train.py experiment=dreamer/atari100k algorithm.dreamer_config.perf.channels_last=false   # NCHW first encoder conv (pre-parity port behaviour)
```
...
as in r2dreamer, which pins whenever replay lives on the CPU. (r2dreamer's
